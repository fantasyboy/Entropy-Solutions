...
namespace AIO.Utilities
{
	...
	internal static class Bools
	{
		...
		{
			name = name.ToLower();
			return name.Contains("attack") && !UtilityClass.NoAttacks.Contains(name) ||
			       UtilityClass.Attacks.Contains(name);
		}
		...
		{
			var sheenLikeBuffNames = new[]
				{"sheen", "LichBane", "dianaarcready", "ItemFrozenFist", "sonapassiveattack", "AkaliTwinDisciplines"};
		...
		public static bool IsTearLikeItemReady(this AIHeroClient unit)
		{
			...
			var tearLikeItemSlot =
				UtilityClass.Player.InventorySlots.FirstOrDefault(s =>
					UtilityClass.TearLikeItems.Contains((ItemID) s.ItemID));
