...
namespace AIO
{
	...
	internal partial class General
	{
		...
		public static void Menu()
		{
			...
			///     Loads the root Menu.
			/// </summary>
			MenuClass.Root = new Menu(UtilityClass.Player.CharName.ToLower(),
				"ExorAIO: " + UtilityClass.Player.CharName, true);
			{
				/// <summary>
				...
				MenuClass.General = new Menu("generalmenu", "General Menu");
				{
					...
					MenuClass.General.Add(new MenuSliderBool("disableaa",
						"Disable AutoAttacks in Combo / If Level >= x", false, 2, 2, 18));
					MenuClass.General.Add(
						new MenuBool("junglesmall", "Cast to small minions too in JungleClear", false));

					if (UtilityClass.Player.MaxMP >= 200)
					{
						MenuClass.General.Add(new MenuBool("nomanagerifblue",
							"Ignore ManaManagers if you have Blue Buff", false));
					}
					...
					MenuClass.PreserveMana = new Menu("preservemanamenu", "Preserve Mana Menu");
					{
						...
						if (championSpellManaCosts != null)
						{
							...
							{
								MenuClass.PreserveMana.Add(new MenuBool(slot.ToString().ToLower(), slot.ToString(),
									false));
							}
						}
						else
						{
							MenuClass.PreserveMana.Add(new MenuSeperator("preserveseparator",
								"Preserve Mana not needed"));
						}
					}
					...
					{
						MenuClass.PreserveSpells.Add(new MenuSeperator("separator",
							"Only works for inside-AA-range targets"));
						MenuClass.PreserveSpells.Add(new MenuSeperator("separator2", "0 = Don't limit."));
						foreach (var slot in UtilityClass.SpellSlots)
						{
							MenuClass.PreserveSpells.Add(new MenuSlider(slot.ToString().ToLower(),
								$"Don't cast {slot} if target killable in X AAs", 0, 0, 10));
						}
					}
