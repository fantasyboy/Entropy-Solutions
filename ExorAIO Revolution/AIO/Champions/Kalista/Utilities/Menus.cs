...
namespace AIO.Champions
{
	...
	internal partial class Kalista
	{
		...
		public void Menus()
		{
			...
			{
				MenuClass.W.Add(new MenuSliderBool("logical", "Check important Jungle spots / if Mana >= x%", true, 50,
					0, 99));
			}
			...
			MenuClass.R = new Menu("r", "Use R to:");
			{
				foreach (var element in RLogics)
				{
					...
					{
						MenuClass.R.Add(new MenuSeperator(element.Value.Item2,
							$"Ally {element.Key} not found, no need for a {element.Value.Item2} option."));
					}
				}

				MenuClass.R.Add(new MenuSliderBool("lifesaver", "Soulbound Lifesaver / If Soulbound Health <= X%", true,
					15, 10, 90));
			}
